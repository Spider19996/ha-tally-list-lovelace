  ...
  render() {
    ...
    if (this.config.show_copy !== false) {
      buttons.push(html`<button
          ...
          style=${this._copyBtnWidth ? `width:${this._copyBtnWidth}px` : ''}
          @click=${this._copyRanking}
          ?disabled=${this._copyDisabled}
        >${this._copyDisabled ? this._t('copied') : this._t('copy_table')}</button>`);
    }
