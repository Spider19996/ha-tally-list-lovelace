...
class TallyListFreeDrinksCard extends LitElement {
  ...
  render() {
    ...
    if (user) {
      ...
      const countLabel = fdT(this.hass, this.config.language, 'count').toLowerCase();
      for (const [drink, entity] of Object.entries(user.drinks)) {
        ...
        if (lower.endsWith(countLabel)) {
          name = name
            .slice(0, -countLabel.length)
            .replace(/[-:\s]+$/i, '')
            .trim();
        } else if (lower.endsWith('anzahl')) {
          name = name
            .slice(0, -'anzahl'.length)
            .replace(/[-:\s]+$/i, '')
            .trim();
        }
