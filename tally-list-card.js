  ...
  render() {
    ...
    if (user) {
      for (const [drink, entity] of Object.entries(user.drinks)) {
        ...
        const name = st?.attributes?.friendly_name || drink;
    ...
    return html`
      <div class="header">
        ${this.config.user_mode === 'auto'
          ? html`<select id="${idUser}" @change=${this._onSelectUser}>
              ${repeat(
                users,
                (u) => u.slug,
                (u) => html`<option value="${u.slug}" ?selected=${
                  u.slug === this.selectedUser
                }>${u.name}</option>`
              )}
            </select>`
          : ''}
        <input
          id="${idSearch}"
          type="text"
          placeholder="${fdT(this.hass, this.config.language, 'search')}"
          ...
          ${repeat(
            ...
            (d) => html`<tr>
                ...
                ${showPrices
                  ...
                  : ''}
                <td>
                  <button data-drink="${d.drink}" @click=${this._dec}>-</button>
                  <span>${pending[d.drink] || 0}</span>
                  <button data-drink="${d.drink}" @click=${this._inc}>+</button>
                </td>
              </tr>`
          )}
        </tbody>
      ...
      <div class="footer">
        ...
        <button class="submit" ?disabled=${
            !this._validComment() || this._pendingSum() === 0
          } @click=${this._submit}>
